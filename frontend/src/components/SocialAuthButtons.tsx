    ...
    <>
      <div className="relative my-6">
        ...
        <div className="relative flex justify-center text-sm">
          <span className="px-2 bg-white text-gray-500">Or {mode === 'signup' ? 'sign up' : 'continue'} with</span>
        </div>
      </div>
        ...
        {enabledProviders.map((provider) => (
          ...
          >
            ...
            <span className="text-gray-700 font-medium">
              {pendingProvider === provider.name ? `Connecting to ${provider.name}...` : `${action} with ${provider.name}`}
            </span>
          </button>
        ))}
      </div>
