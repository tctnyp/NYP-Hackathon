        ...
        {['all', 'not_started', 'in_progress', 'overdue', 'completed'].map((status) => (
          <button
            ...
            className={`px-4 py-2 rounded-lg capitalize ${
              filter === status ? 'bg-primary-600 text-white' : 'bg-white text-gray-700 border'
            }`}
          >
            {status.replace('_', ' ')}
          </button>
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {tasks.length === 0 ? (
            ...
            tasks.map((task) => (
              <div key={task.task_id} className="bg-white p-6 rounded-lg border hover:shadow-md transition">
