  ...
  const handleCalendarAction = async (action: 'enable' | 'sync' | 'disable') => {
    ...
    try {
      if (action === 'enable') {
        ...
        setCalendarMessage('Google Calendar is up to date.');
      } else {
          ...
          <div className="flex min-w-0 gap-3">
            ...
            <div>
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span className={calendarSync.enabled ? 'rounded-full bg-green-100 px-2 py-0.5 text-xs font-semibold text-green-700' : 'rounded-full bg-gray-100 px-2 py-0.5 text-xs font-semibold text-gray-600'}>
                  {calendarSync.enabled ? 'On' : calendarSync.status === 'disable_pending' ? 'Cleanup pending' : 'Off'}
                </span>
              </div>
