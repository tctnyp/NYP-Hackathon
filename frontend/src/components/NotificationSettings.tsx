    ...
    <section className="card task-reminder-settings" aria-labelledby="notifications-heading">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="flex min-w-0 items-start gap-3">
          ...
          <div><h2 id="notifications-heading" className="text-lg font-semibold">Task reminders</h2><p className="reminder-copy mt-1 max-w-xl text-sm leading-6">Get a helpful reminder about your top task when you switch to another app. For now, reminders stop if you fully close Munera.</p></div>
        </div>
      ...
      <div className="mt-5 rounded-2xl border p-4" style={{ borderColor: 'var(--app-border)', background: 'var(--app-bg)' }}>
        {!isInstalled && isIosDevice ? (
          <div className="flex items-start gap-3"><Share size={20} className="mt-0.5 shrink-0 text-primary-600" /><div><p className="text-sm font-semibold">Add Munera to your Home Screen</p><p className="mt-1 text-sm leading-6 text-gray-600">In Safari, tap Share, choose “Add to Home Screen,” then open Munera from the new icon. On iPhone and iPad, notifications require iOS or iPadOS 16.4 or later.</p></div></div>
        ) : !notificationsSupported ? (
          <p className="text-sm text-gray-600">This browser or device doesn’t support notifications.</p>
        ) : !isInstalled ? (
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><p className="text-sm text-gray-600">Install Munera to turn on task reminders.</p>{canInstall && <button type="button" className="btn-primary inline-flex items-center justify-center gap-2" onClick={() => void install()}><Download size={17} /> Install app</button>}</div>
        ) : !notificationDeliveryReady ? (
          <p className="text-sm leading-6 text-amber-700">Notifications aren’t ready yet. Refresh Munera and try again.</p>
        ) : notificationPermission === 'denied' ? (
          <p className="text-sm leading-6 text-amber-700">Notifications are blocked in your device settings. Allow notifications for Munera, then come back here.</p>
        ) : notificationsEnabled ? (
